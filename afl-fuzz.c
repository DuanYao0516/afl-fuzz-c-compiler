...
void lexer_mutate(u8 **out_buf, char **original, char **replacement,
                  size_t *pos, int target, int temp_len)
{
    ...
    cur_line_num = 1;
    lex_origin = original;
    lex_repl = replacement;
    lex_pos = *pos;
    target_type = target;
    char *input_string = *out_buf;
    // yy_scan_string(input_string); // 以字符串作为输入流
    YY_BUFFER_STATE buffer = yy_scan_bytes(input_string, temp_len);
...
static int use_mutation_tool(u8 **out_buf, s32 *temp_len)
{
  ...
  {
    pos = UR((*temp_len) - 1);
    int choice = UR(140); // 多种变异替换算子，任选一种
    switch (choice)
    {
    ...
    strncpy(original, ">>", MAX_MUTANT_CHANGE);
      strncpy(replacement, "<<", MAX_MUTANT_CHANGE);
      break;
      /*id_replace(out_buf, temp_len, &original, &replacement, pos);
      break;*/
    /* Replace integer */
    case 79:
    strncpy(original, ">>", MAX_MUTANT_CHANGE);
      strncpy(replacement, "<<", MAX_MUTANT_CHANGE);
      break;
      /*int_replace(out_buf, temp_len, &original, &replacement, pos, "0");
      break;*/
    }
    if(choice >= 80 && choice < 90)
	lexer_mutate(out_buf, &original, &replacement, &pos, ID, *temp_len);
    else if(choice >= 90 && choice < 100)
	lexer_mutate(out_buf, &original, &replacement, &pos, INTEGER, *temp_len);
    else if(choice >= 100 && choice < 110)
	lexer_mutate(out_buf, &original, &replacement, &pos, TYPE_SPECIFIER, *temp_len);
    else if(choice >= 110 && choice < 120)
	lexer_mutate(out_buf, &original, &replacement, &pos, FLOAT, *temp_len);
    else if(choice >= 120 && choice < 130)
	lexer_mutate(out_buf, &original, &replacement, &pos, STRING, *temp_len);
    else if(choice >= 130 && choice < 140)
	lexer_mutate(out_buf, &original, &replacement, &pos, CHAR, *temp_len);
...
EXP_ST void detect_file_args(char **argv)
{
  ...
  while (argv[i])
  {
    ...
    if (aa_loc)
    {
      ...
      if (!out_file)
        out_file = alloc_printf("%s/a.c", out_dir);
...
stop_fuzzing:
  ...
  OKF("We're done here. Have a nice day! See u next time^^!!!\n");
